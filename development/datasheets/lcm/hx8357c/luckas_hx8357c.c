...
{
	send_ctrl_cmd(0xB9);					// LCD Reset	LuckAs
	send_data_cmd(0xFF);					//
	send_data_cmd(0x83);					//
	send_data_cmd(0x57);					//

	MDELAY(50);								// 5 (maybe fixme)	LuckAs

	send_ctrl_cmd(0xB6); //					// 
	send_data_cmd(0x25); //VCOMDC  2c 		// VCOMDC

	send_ctrl_cmd(0x35); // TE ON 			// TE ON

	send_ctrl_cmd(0xB0);					//
//	send_data_cmd(0x68); //70Hz				// + Disable 70Hz	LuckAs
	send_data_cmd(0x66); 					//60Hz	From Datasheet	LuckAs

	send_ctrl_cmd(0xCC); //Set Panel		// Set Panel
	send_data_cmd(0x09); //					//

	send_ctrl_cmd(0xB1); //					//
	send_data_cmd(0x00); //DP				// DP
	send_data_cmd(0x15); //BT				// BT
//	send_data_cmd(0x1C); //VSPR1C3F			// + Disable 	LuckAs
	send_data_cmd(0x1E); //VSPR1C3F			// VSPR - add from Datasheet	LuckAs
//	send_data_cmd(0x1C); //VSNR1C3F			// + Disable 	LuckAs
	send_data_cmd(0x1E); //VSNR1C3F			// VSNR - add from Datasheet	LuckAs
	send_data_cmd(0x83); //AP				// AP
	send_data_cmd(0x48); //FS 				// FS

	send_ctrl_cmd(0xC0); //					//
	send_data_cmd(0x50); //OPON				//OPON
	send_data_cmd(0x50); //OPON				//OPON
	send_data_cmd(0x01); //STBA				//STBA
	send_data_cmd(0x3C); //STBA				//STBA
//	send_data_cmd(0x1E); //STBA				// + Disable 	LuckAs
	send_data_cmd(0xC8); //STBA				// STBA - add from Datasheet	LuckAs
	send_data_cmd(0x08); //GEN				//GEN

	send_ctrl_cmd(0xB4); //					//
//	send_data_cmd(0x02); //NW				// + Disable 	LuckAs
	send_data_cmd(0x01); //NW				// NW - add from Datasheet	LuckAs
	send_data_cmd(0x40); //RTN				//RTN
	send_data_cmd(0x00); //DIV				//DIV
	send_data_cmd(0x2A); //DUM				//DUM
	send_data_cmd(0x2A); //DUM				//DUM
	send_data_cmd(0x0D); //GDON				//GDON
//	send_data_cmd(0x78); //GDOFF			// + Disable 	LuckAs
	send_data_cmd(0x8F); //GDOFF			//0x8F GDOFF - add from Datasheet	LuckAs

//	send_data_cmd(0xB5); //					// 			- add from Datasheet	LuckAs
//	send_data_cmd(0x00); //					// VREF 	- add from Datasheet	LuckAs
//	send_data_cmd(0x00); //					// NVREF 	- add from Datasheet	LuckAs


	send_ctrl_cmd(0xE0); //					// 
//	send_data_cmd(0x02); // //  1			// + Disable 	LuckAs
//	send_data_cmd(0x08); // // 2			// + Disable 	LuckAs
//	send_data_cmd(0x11); //3				// + Disable 	LuckAs
//	send_data_cmd(0x23); //4				// + Disable 	LuckAs
//	send_data_cmd(0x2C); //5				// + Disable 	LuckAs
//	send_data_cmd(0x40); //6				// + Disable 	LuckAs
//	send_data_cmd(0x4A); //7				// + Disable 	LuckAs
//	send_data_cmd(0x52); //8				// + Disable 	LuckAs
//	send_data_cmd(0x48); //9				// + Disable 	LuckAs
//	send_data_cmd(0x41); //10				// + Disable 	LuckAs
//	send_data_cmd(0x3C); //11				// + Disable 	LuckAs
//	send_data_cmd(0x33); //12				// + Disable 	LuckAs
//	send_data_cmd(0x2E); //13				// + Disable 	LuckAs
//	send_data_cmd(0x28); //14				// + Disable 	LuckAs
//	send_data_cmd(0x27); //15				// + Disable 	LuckAs
//	send_data_cmd(0x1B); //16				// + Disable 	LuckAs
//	send_data_cmd(0x02); //17 v1			// + Disable 	LuckAs
//	send_data_cmd(0x08); //18				// + Disable 	LuckAs
//	send_data_cmd(0x11); //19				// + Disable 	LuckAs
//	send_data_cmd(0x23); //20				// + Disable 	LuckAs
//	send_data_cmd(0x2C); //21				// + Disable 	LuckAs
//	send_data_cmd(0x40); //22				// + Disable 	LuckAs
//	send_data_cmd(0x4A); //23				// + Disable 	LuckAs
//	send_data_cmd(0x52); //24				// + Disable 	LuckAs
//	send_data_cmd(0x48); //25				// + Disable 	LuckAs
//	send_data_cmd(0x41); //26				// + Disable 	LuckAs
//	send_data_cmd(0x3C); //27				// + Disable 	LuckAs
//	send_data_cmd(0x33); //28				// + Disable 	LuckAs
//	send_data_cmd(0x2E); //29				// + Disable 	LuckAs
//	send_data_cmd(0x28); //30				// + Disable 	LuckAs
//	send_data_cmd(0x27); //31				// + Disable 	LuckAs
//	send_data_cmd(0x1B); //32				// + Disable 	LuckAs
//	send_data_cmd(0x00); //33				// + Disable 	LuckAs
//	send_data_cmd(0x01); //34				// + Disable 	LuckAs

	send_data_cmd(0x00);					// 1	- add from Datasheet	LuckAs
	send_data_cmd(0x02);					// 2	- add from Datasheet	LuckAs
	send_data_cmd(0x06);					// 3	- add from Datasheet	LuckAs
	send_data_cmd(0x15);					// 4	- add from Datasheet	LuckAs
	send_data_cmd(0x20);					// 5	- add from Datasheet	LuckAs
	send_data_cmd(0x38);					// 6	- add from Datasheet	LuckAs
	send_data_cmd(0x43);					// 7	- add from Datasheet	LuckAs
	send_data_cmd(0x4D);					// 8	- add from Datasheet	LuckAs
	send_data_cmd(0x4C);					// 9	- add from Datasheet	LuckAs
	send_data_cmd(0x44);					// 10	- add from Datasheet	LuckAs
	send_data_cmd(0x3F);					// 11	- add from Datasheet	LuckAs
	send_data_cmd(0x36);					// 12	- add from Datasheet	LuckAs
	send_data_cmd(0x30);					// 13	- add from Datasheet	LuckAs
	send_data_cmd(0x2B);					// 14	- add from Datasheet	LuckAs
	send_data_cmd(0x28);					// 15	- add from Datasheet	LuckAs
	send_data_cmd(0x1B);					// 16	- add from Datasheet	LuckAs
	send_data_cmd(0x00);					// 17 v1- add from Datasheet	LuckAs
	send_data_cmd(0x02);					// 18	- add from Datasheet	LuckAs
	send_data_cmd(0x06);					// 19	- add from Datasheet	LuckAs
	send_data_cmd(0x15);					// 20	- add from Datasheet	LuckAs
	send_data_cmd(0x20);					// 21	- add from Datasheet	LuckAs
	send_data_cmd(0x38);					// 22	- add from Datasheet	LuckAs
	send_data_cmd(0x43);					// 23	- add from Datasheet	LuckAs
	send_data_cmd(0x4D);					// 24	- add from Datasheet	LuckAs
	send_data_cmd(0x4C);					// 25	- add from Datasheet	LuckAs
	send_data_cmd(0x44);					// 26	- add from Datasheet	LuckAs
	send_data_cmd(0x3F);					// 27	- add from Datasheet	LuckAs
	send_data_cmd(0x36);					// 28	- add from Datasheet	LuckAs
	send_data_cmd(0x30);					// 29	- add from Datasheet	LuckAs
	send_data_cmd(0x2B);					// 30	- add from Datasheet	LuckAs
	send_data_cmd(0x28);					// 31	- add from Datasheet	LuckAs
	send_data_cmd(0x1B);					// 32	- add from Datasheet	LuckAs
	send_data_cmd(0x00);					// 33	- add from Datasheet	LuckAs
	send_data_cmd(0x01);					// 34	- add from Datasheet	LuckAs
	
//	send_ctrl_cmd(0x3A);   					// + Disable 	LuckAs
//	send_data_cmd(0x55);   					// (262K maybe fixme) + Disable 	LuckAs
	send_ctrl_cmd(0x3A);   					// - add from Datasheet	LuckAs
	send_data_cmd(0x06);   					// 262K - add from Datasheet	LuckAs

	send_ctrl_cmd(0x36);					// NONE
	send_data_cmd(0x00);					// NONE

	send_ctrl_cmd(0x2A);					// NONE
	send_data_cmd(0x00);					// NONE
	send_data_cmd(0x00);					// NONE
	send_data_cmd(0x01);					// NONE
	send_data_cmd(0x3F);					// NONE

	send_ctrl_cmd(0x2B);					// NONE
	send_data_cmd(0x00);					// NONE
	send_data_cmd(0x00);					// NONE
	send_data_cmd(0x01);					// NONE
	send_data_cmd(0xDF);					// NONE

	send_ctrl_cmd(0x11); // SLPOUT 			//Exit Sleep
	MDELAY(120);   							// 150 maybe need fixme LuckAs

	send_ctrl_cmd(0x29); // display on		// display on
	MDELAY(50);								// 25 maybe need fixme LuckAs
	send_ctrl_cmd(0x2C);      				// 
}
// LuckAs add from Datasheet
// ---------------------------------------------
